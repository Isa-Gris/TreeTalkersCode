...
####
## OPTION B: Having the data on *.txt files from TT+ --------------
####

#' Inputs: TXT files dowloaded directly from TT+
#' Outputs: One database appended to C:/TreeTalkerDB/Raw_fullDB.csv

rm(list=ls())

# (B.1) Write/modify the input directory, folder with TXT files from TTs (example):
setwd("C:/Users/Isabel/Documents/TFM - Tree Talkers/Códigos de R/DatosTFM")


# (B.2) Run "DownloadTTDB_B" to save the function:

DownloadTTDB_B <- function() {
...
ReadTXTData <- function() {
  ...
  cat("Cleaning the",
    dbname,
    "file and organizing the data into the C:/TreeTalkerDB/ttDB.csv file.",
    "\n"
  )
  ...
  # We have 2 different types of data from TT+ (4D and 55 are the same)
  dbString <- list("4D", "49", "55")
  ...
  # merge TT different strings (4D/55 and 49)
  ttDB <- cbind(db55, db49)
  ttDB <- ttDB[,c(2,4,1,3,5:22,24,26:39)]

  # add the TTId to the db
  TTsn <- as.character(ttDB$TTsn)
  TTid <- substr(ttDB$Location,1,nchar(ttDB$Location)-4)
...
# There are no TTClouds in those txt files
...
dbTT <- ttDB  # Copy TT's DB from previous step.
              # Filter by TTid here or at the end if it is necessary

checkTT <- function(dbTT) {
  whichDay <- as.Date(Sys.Date())

  cat("Checking the status of TTs on", paste0(whichDay), "\n")
  cat("Check out the output as the full database in C:/TreeTalkerDB/TTResults as",
      paste0(whichDay, "_TTDB.csv"), "\n")
  ...
  for (j in 1:length(ID)) {
    ...
    n = 7
    m = 0
    ts_filt <- savitzkyGolay(ts, m, p, n)
  ...
  stwc_min <-
    aggregate(dbTT$stwc, list(as.character(dbTT$TTid)), min)
  ...
  dbTT_Results <- dbTT[,c(
    "TTid", "TTsn", "timeStamp", "Location", "relativeHumidity",
  ...
  write.table(dbTT_Results,
              file.path(out_dir, paste0(whichDay, "_TTDB.csv")),
              row.names = FALSE,
              sep = ";")
...
#dbTT_alert <- dbTT_alert[dbTT_alert$timeStamp >= Date0 & dbTT_alert$timeStamp <= Date1,]
...
Plot <- function(TTdb_today, Date0, Date1) {
  ...
  #Date2 <- Date1 # data will include the entire day
  data_new <-
...
#' Input the folder path with the "TTReportTXT.Rmd" file downloaded
#' to create the report with RMarkdown (TTReport.pdf)

render_path <- file.path("C:/Users/Isabel/Downloads", "TTReportTXT.Rmd")
